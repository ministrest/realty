...
$(document).ready(function() {
	...
	var dataArray = [];
	var object_id = document.getElementById("object[id_object]").value;

// Загрузка изображений на сервер
	$('.deleteimg').click(function() {
		if(confirm('Вы уверены, что хотите удалить изображение?')) {
			var idimg = $(this).attr('id');
			$.post('/admin/objects/deleteimg/' + idimg, function (data) {
			});
			$(this).parent().remove();
		}
		return false;
	...
	$('#upload-button .upload').click(function() {
		...
		$.each(dataArray, function(index, file) {	
			...
			$.post('/admin/objects/addphoto/'+object_id, dataArray[index], function(data) {
				...
				if(dataSplit[1] == 'загружен успешно') {
					$('#uploaded-files').append('<li><a href="/uploads/objects/'+dataSplit[0]+'">'+fileName+'</a> загружен успешно</li>');
								
				} else {
					$('#uploaded-files').append('<li><a href="/uploads/objects/'+data+'. Имя файла: '+dataArray[index].name+'</li>');
				}
