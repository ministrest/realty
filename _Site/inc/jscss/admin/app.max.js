...
function _init() {
    ...
    $.AdminLTE.layout = {
        ...
        $(document).on("click", a + " li a", function (a) {
            ...
            }), e.parent("li").removeClass("active"),Cookies.set("treeview["+e.parent("li").attr('id')+"]",0,{expires:1}); else if (e.is(".treeview-menu") && !e.is(":visible")) {
                var f = d.parents("ul").first()/*, g = f.find("ul:visible").slideUp(c)*/;
                //g.removeClass("menu-open");
                var h = d.parent("li");
                e.slideDown(c, function () {
                    e.addClass("menu-open"), /*f.find("li.active").removeClass("active"),*/ h.addClass("active"),Cookies.set("treeview["+h.attr('id')+"]",1,{expires:1}), b.layout.fix()
                })
            }
